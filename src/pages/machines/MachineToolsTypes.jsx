...
const MachineToolsTypes = ({ id = null, data = {}, rwd = defaultRwd }) => {
  // prawa strona – słownik opcji, w trybie readOnly (odchudzony schema)
  const optionsEntity = useEntity({
    endpoint: '/tools_types/',
    entityName: 'ToolsTypes',
    readOnly: true,
  });
  ...
  const assignedEntity = useEntity({
    endpoint: '/machines_tool_types/',
    entityName: 'MachinesToolsTypes',
    query: { machine_id: id },
  });
  ...
  const handleAdd = useCallback(
    async ({ mappedItemData, prevElementData }) => {
      ...
      // 🔑 payload zgodny ze schematem MachineToolsTypess
      const payload = {
        machine_id: id,
        tool_type_id: mappedItemData.id,
        is_required: true,
        notes: '',
      };
      ...
      try {
        ...
        console.error('Error creating MachineToolsTypes row', e);
      }
    },
  ...
  return (
    ...
    >
      <Mapper
        entityName='MachineToolsTypesMapper'
        ownerLabel="Typy Narzędzi Maszyny"
        owner={{ id }}
...
export default MachineToolsTypes;
