...
const ResourceRelations = ({ id = null, data = {}, rwd = defaultRwd }) => {
  ...
  const assignedEntity = useEntity({
    endpoint: '/resources_relations/',
    entityName: 'CombinedResources',
    query: { parent_resource_id: id },
  });
  ...
  const handleAdd = useCallback(
    async ({ mappedItemData, prevElementData }) => {
      ...
      const payload = {
        child_resource_id: mappedItemData.id,
  ...
  if(!data.is_combined){
    return <Alert severity='info'>{data.name} {data.category} - nie jest zasóbem złożonym</Alert>
  }
  ...
  return (
    ...
    >
      <Mapper
        ...
        distinct={true}
        distinctField='child_resource_id'
